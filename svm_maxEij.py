...
def cal_err(k, os):
    fxk = sum(os.alphas * os.labels * os.data.dot(os.data[k])) + os.b
...
def inner(i, os):
    ...
    if (os.labels[i] * ei < -os.toler and os.alphas[i] < os.C) or (os.labels[i] * ei > os.toler and os.alphas[i] > 0):
        ...
        eta = 2.0 * os.data[i].dot(os.data[j]) - os.data[i].dot(os.data[i]) - os.data[j].dot(os.data[j])
        ...
        #步骤7：计算b1，b2， b
        b1 = os.b - ei - os.labels[i] * (os.alphas[i] - alpha_i_old) * os.data[i].dot(os.data[i]) - os.labels[j] * (
                os.alphas[j] - alpha_j_old) * os.data[i].dot(os.data[j])
        b2 = os.b - ej - os.labels[i] * (os.alphas[i] - alpha_i_old) * os.data[i].dot(os.data[j]) - os.labels[j] * (
                os.alphas[j] - alpha_j_old) * os.data[j].dot(os.data[j])
