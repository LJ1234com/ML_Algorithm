...
def selectJ(i, oS, Ei):
    ...
    # validEcacheList = np.nonzero((oS.alphas.A[0] > 0) * (oS.alphas.A[0] < oS.C))[0]  # 在违反KKT中去找
...
def testRbf(k1=1.3):
    ...
    Returns:
        无
    """
    dataArr, labelArr = loadDataSet('svm_kernel1.txt')  # 加载训练集
    ...
    dataArr, labelArr = loadDataSet('svm_kernel1.txt')  # 加载测试集
    errorCount = 0
    datMat = np.mat(dataArr);
    labelMat = np.mat(labelArr).transpose()
    m, n = np.shape(datMat)
